# go-boot-gin 项目开发规范文档

go-boot-gin 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 Gin Web 框架集成模块。本模块将 Gin 框架无缝集成到 go-boot 的 IoC 容器和自动配置体系中，遵循 go-boot 项目的开发规范。
...
- **集成模块**：go-boot-gin 是 go-boot 的 Web 层集成，将 Gin 作为 HTTP 服务器实现
- **规范继承**：完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 Gin Engine 注册为 go-boot 容器中的 Bean
- 提供基于 go-boot 自动配置的 Gin 启动器
- 实现 `net.Server` 接口的 Gin 适配器
- 支持路由注册、中间件配置的声明式配置
...
- **Web 框架**：集成 Gin（`github.com/gin-gonic/gin`）
- **职责边界**：仅负责 Gin 集成，不包含其他业务逻辑
- **示例代码**：统一放在 `examples/` 目录，演示 Gin 集成用法

### 2.2 go-boot-gin 核心包结构

| 包 | 说明 | 接口定义 |
|---|------|----------|
| `gin/` | Gin Engine 集成（自动配置、Bean 注册） | `gin.Engine` |
| `server/` | Gin Server 适配器（实现 net.Server） | `server.GinServer` |
| `router/` | 路由注册辅助（声明式路由配置） | `router.Router` |
| `middleware/` | 中间件配置（日志、恢复、CORS 等） | 各类中间件函数 |

### 2.3 go-boot 核心包参考

go-boot-gin 依赖 go-boot 的以下核心包：
...
go-boot-gin 遵循 go-boot 的接口抽象原则，所有集成层通过核心框架中的接口抽象定义，实现运行时互换：
